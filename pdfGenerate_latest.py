...
def merge_pages(front_page, back_page):
    ...
    width = front_page.mediabox.width
    height = front_page.mediabox.height
...
async def main():
    ...
    front_pdf = PyPDF2.PdfReader(open(pdf_front_card, 'rb'))
    back_pdf = PyPDF2.PdfReader(open(pdf_back_card, 'rb'))

    front_page = front_pdf.pages[0]
    back_page = back_pdf.pages[0]

    # 두 명함 이미지를 한 페이지에 나란히 배치
    for page_num in range(len(front_pdf.pages)):
        page = front_page
        pdf_writer.add_page(page)

    for page_num in range(len(back_pdf.pages)):
        page = back_page
        pdf_writer.add_page(page)
